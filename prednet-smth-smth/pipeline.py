...
#from str_tensor import Str_Tensor
# sigma = (1.5,1.5,1.5)
# rho = 5.5
########################################################################################################################
...
parser.add_argument('--weight_dir', type=str, default="G:/",
                    help="Directory for saving trained weights and model")#default=os.path.join(os.getcwd(), "model")
parser.add_argument('--result_dir', type=str, default="G:/",
...
parser.add_argument("--shutdown", help="Shutdown the system at the end of the pipeline, by default it is set to false", action="store_true")
...
parser.add_argument('--extrap_start_time', type=int, default=None,
                    help="starting at this time step, the prediction from "
                         ...
                         "actual input")
parser.add_argument('--plot_orientation_graph', 
                    help="Plots the Frame wise Dominant Orientation direction graphs for the test data.", 
                    action="store_true")
#parser.add_argument('--count', type=int, default=0, help="if you plan on running all of the diffent fps simulations continuosly use the count to transmit a signal to the next execution.")
args = parser.parse_args()
########################################################################################################################

############################################# Common globals for all modes #############################################
f_rate=args.csv_path.split("/")[1]
if(args.frame_selection=="dynamic-fps"):
    args.weight_dir=args.weight_dir+f_rate+"_Simulations/Dynamic_fps/"+"Models"
    args.result_dir=args.result_dir+f_rate+"_Simulations/Dynamic_fps/"+"Results"
else:    
    args.weight_dir=args.weight_dir+f_rate+"_Simulations/Normal/"+"Models"
    args.result_dir=args.result_dir+f_rate+"_Simulations/Normal/"+"Results"
...
#assert args.fps in [1, 2, 3, 6, 12], "allowed values for fps are [1,2,3,6,12] for this dataset. But given {}".format(args.fps)
...
if args.train_model_flag:
    ...
    if(args.multitask_flag):
        ...
        # Configuring the model
        prednet = PredNet(stack_sizes, r_stack_sizes, args.a_filt_sizes, args.ahat_filt_sizes, args.r_filt_sizes,
                          output_mode=output_mode, strided_conv_pool=args.strided_conv_pool, return_sequences=True)#, report_tensor_allocations_upon_oom=True
    ...
    callbacks.append(ModelCheckpoint(filepath=weight_file, monitor='val_loss', verbose=1,
                                      ...
                                      mode='auto', period=period))# Updated Period argument to Save_freq to comply with Keras that comes with TF-2
    ...
    print("Steps_per_epoch_val: "+str(steps_per_epoch_val))
    ...
    with open(history_file, 'w') as f:
        json.dump(str(history.history), f, sort_keys=True,  indent=4)
...
if args.evaluate_model_flag:
    ...
    for weight_file in weight_files:
        print(weight_file)
        weight_file=weight_file.replace(os.sep,'/')
 ...
 is not trained with the multitask_flag on. Please cross check.".format(weight_file)) 
        ...
        #         #print(test_data.id.to_markdown())
        #         if(test_data[test_data.template.isin(lbls)].empty==False):
        #             #print(test_data.to_markdown())
        #             fn = lambda obj: obj.loc[np.random.choice(obj.index, args.plots_per_grp, True),:]
        #             #rows = test_data[test_data.template.isin(lbls)].sample(n=args.plots_per_grp,random_state=123)
        #             rows = test_data.groupby('Template', as_index=False).apply(fn)
        #     else:# specific videos
        #         rows = test_data[test_data.id == lbls] 
        #     #print("Rows: "+str(rows.dtypes))
            
        #     #save names in the df for creating plot names later    
        #     try:
        #         rows
        #     except NameError:#Checking to see if the Test_data batch has cleared the specific category of videos defined in sub_grps variables.
        #         continue
        #     else:
        #         #rows["name"]=name
        #         fn = lambda obj: obj.loc[np.random.choice(obj.index, args.plots_per_grp, True),:]
        ...
        fn = lambda obj: obj.loc[np.random.choice(obj.index, args.plots_per_grp, True),:]
        rows = test_data.groupby('template', as_index=False).apply(fn)
        test_data_for_plt = test_data_for_plt.append(rows, ignore_index=True)#Randomly selecting a video for each template within the Test_data
        #print("test_data_for_plt info : "+str(test_data_for_plt.info()))
        test_data_for_plt.to_csv(path_or_buf=args.result_dir+"/Test_data_for_plt_used_for_"+filename+".csv")
        total_vids_to_plt = len(test_data_for_plt)
        # print("Test Data info: "+str(test_data.columns))
        # test_data.to_csv(path_or_buf=args.result_dir+"Test_data_used_for_"+filename+".csv")
        # total_vids_to_plt =len(test_data)
        # exit()
  ...
  #######################################################################################################################
        ...
        for i in range(total_vids_to_plt):
             ...
             if args.extra_plots_flag:
                 fig, ax = plt.subplots(ncols=1, nrows=20, sharex=True, figsize=(time_steps, 35 * aspect_ratio),
             ...
             if(args.multitask_flag):
                 title = "ID {}. LABEL: {}, PRED: {} Max. Dominant Orientation Direction:{} Deg., Min Dominant Orientation Direction:{} Deg.".format(
                 test_data_for_plt.loc[i,'id'], # Or test_data_for_plt
                 test_data_for_plt.loc[i,'label'],#Or test_data_for_plt
                 lbl_pred_final_template[i], max(Dom_Ori_list).round(), min(Dom_Ori_list).round())
             else:
                 title = "ID {}. LABEL: {}, Max. Dominant Orientation Direction:{} Deg., Min Dominant Orientation Direction:{} Deg.".format(
                 test_data_for_plt.loc[i,'id'], 
                 test_data_for_plt.loc[i,'label'], max(Dom_Ori_list).round(), min(Dom_Ori_list).round())
            ...
            #Creating Plot with Cross correlation coeffecients
             ...
             x=[args.im_width/2+j*args.im_width for j in range(time_steps)]
            ...
            ######################################### Extra plot #############################################
             ...
             plt.savefig(plot_save_dir + "/" + test_data_for_plt.loc[i,'template']+"_"+test_data_for_plt.loc[i,'label']+"_"+str(test_data_for_plt.loc[i,'id']) + '.png')
...
n = text_file.write("Time elapsed for complete pipeline: {:.0f}h:{:.0f}m:{:.0f}s".format(time_elapsed // 3600, (time_elapsed // 60) % 60, time_elapsed % 60))
text_file.close()
# # url_list=["E:/fps3/data.csv", "F:/fps6/data.csv", "G:/fps10/data.csv", "D:/fps12/data.csv"]
# # dynamic="dynamic-fps"
if(args.shutdown):
    os.system("shutdown /s /t 1")
#else:
#    os.system("python pipeline.py --csv_path D:/fps12/data.csv --weight_dir H:/PredNet_Critic/ --result_dir H:/PredNet_Critic/ --crop_grp 2 --batch_size 32 --nframes 24 --fps 12 --im_height 48 --im_width 80 --evaluate_model_flag --extra_plots_flag  --samples_per_epoch 1500 --samples_per_epoch_val 300 --nb_epochs 150 --early_stopping_patience 80 --n_chan_layer 32 48 64 128 --a_filt_sizes 5 5 5 5 --ahat_filt_sizes 3 3 3 3 3 --r_filt_sizes 3 3 3 3 4 --layer_loss 1 0 0 0 0 --data_split_ratio 0.002 --model_checkpoint 1 --samples_test 100 --seed 123 --count 0 --train_model_flag --frame_selection 'dynamic-fps' --shutdown")
#     if args.count%2==0:
#         os.system("python pipeline.py --csv_path "+url_list[args.count//2]+" --weight_dir H:/PredNet_Critic/ --result_dir H:/PredNet_Critic/ --crop_grp 2 --batch_size 32 --nframes 24 --fps "+str(url_list[args.count//2].split("/")[1].replace("fps",''))+" --im_height 48 --im_width 80 --evaluate_model_flag --extra_plots_flag  --samples_per_epoch 1500 --samples_per_epoch_val 300 --nb_epochs 150 --early_stopping_patience 80 --n_chan_layer 32 48 64 128 --a_filt_sizes 5 5 5 5 --ahat_filt_sizes 3 3 3 3 3 --r_filt_sizes 3 3 3 3 4 --layer_loss 1 0 0 0 0 --data_split_ratio 0.002 --model_checkpoint 1 --samples_test 100 --seed 123 --train_model_flag --frame_selection "+dynamic+" --count "+str(args.count+1))
#     elif args.count==7:
#         os.system("python pipeline.py --csv_path "+args.csv_path+" --weight_dir H:/PredNet_Critic/ --result_dir H:/PredNet_Critic/ --crop_grp 2 --batch_size 32 --nframes 24 --fps "+str(args.fps)+" --im_height 48 --im_width 80 --evaluate_model_flag --extra_plots_flag  --samples_per_epoch 1500 --samples_per_epoch_val 300 --nb_epochs 150 --early_stopping_patience 80 --n_chan_layer 32 48 64 128 --a_filt_sizes 5 5 5 5 --ahat_filt_sizes 3 3 3 3 3 --r_filt_sizes 3 3 3 3 4 --layer_loss 1 0 0 0 0 --data_split_ratio 0.002 --model_checkpoint 1 --samples_test 100 --seed 123 --train_model_flag --frame_selection "+dynamic+" --shutdown")
#     else:
#         os.system("python pipeline.py --csv_path "+args.csv_path+" --weight_dir H:/PredNet_Critic/ --result_dir H:/PredNet_Critic/ --crop_grp 2 --batch_size 32 --nframes 24 --fps "+str(args.fps)+" --im_height 48 --im_width 80 --evaluate_model_flag --extra_plots_flag  --samples_per_epoch 1500 --samples_per_epoch_val 300 --nb_epochs 150 --early_stopping_patience 80 --n_chan_layer 32 48 64 128 --a_filt_sizes 5 5 5 5 --ahat_filt_sizes 3 3 3 3 3 --r_filt_sizes 3 3 3 3 4 --layer_loss 1 0 0 0 0 --data_split_ratio 0.002 --model_checkpoint 1 --samples_test 100 --seed 123 --train_model_flag --count "+str(args.count+1))
